...
export function useRhythmEngine() {
  ...
  const getRamp = useCallback((timeSec: number) => {
    const end = songRef.current?.notes[songRef.current.notes.length - 1]?.time ?? 210;
    return Math.min(Math.max(timeSec / end, 0), 1);
  }, []);

  const getTimingWindowsAt = useCallback((timeSec: number) => {
    const ramp = getRamp(timeSec);
    const lerp = (a: number, b: number) => a + (b - a) * ramp;
    return {
      perfect: lerp(0.08, 0.022),
      great: lerp(0.14, 0.055),
      good: lerp(0.22, 0.095),
      miss: lerp(0.30, 0.13),
    };
  }, [getRamp]);
  ...
  /* ── Finish song ── */
  const finishSong = useCallback((didClear: boolean) => {
    if (phaseRef.current === 'results') return;
    const transport = Tone.getTransport();
    const survived = transport.seconds;
    transport.stop();
    transport.cancel();
  ...
  const startSong = useCallback(async (selectedSong: Song, diff: Difficulty) => {
    ...
    selectedSong.notes.forEach((note, i) => {
      const missTime = note.time + getTimingWindowsAt(note.time).miss;
      transport.schedule(() => {
        ...
        if (block && block.grade === null) {
          ...
          setGradeSeq((s) => s + 1);
          setHealth((h) => {
            const next = Math.max(0, h - 14);
            if (next <= 0) finishSong(false);
            return next;
          });
        }
      }, missTime);
  ...
  }, [startFrameLoop, finishSong, getTimingWindowsAt]);

  /* ── Judge a tap input ── */
  const judgeTap = useCallback((lane: number) => {
    if (!songRef.current) return;
    const now = Tone.getTransport().seconds;
    const windows = getTimingWindowsAt(now);
    ...
    if (grade === 'miss') {
      ...
      setScore((s) => s + points * (1 + Math.floor(combo / 10) * 0.1 + ramp * 0.6));
      if (grade === 'perfect') setPerfects((p) => p + 1);
      else if (grade === 'great') setGreats((g) => g + 1);
      else setGoods((g) => g + 1);

      setHealth((h) => {
        const recover = grade === 'perfect' ? 2.8 : grade === 'great' ? 2 : 1.2;
        return Math.min(100, h + recover);
      });
    }
    setLastGrade(grade);
    setGradeSeq((s) => s + 1);
  }, [combo, finishSong, getRamp, getTimingWindowsAt]);
  ...
  const getResult = useCallback((): SongResult | null => {
    ...
    return result;
  }, [song, difficulty, score, maxCombo, perfects, greats, goods, misses, survivedSeconds]);
