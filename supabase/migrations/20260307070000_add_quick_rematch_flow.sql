alter table public.multiplayer_match_players
  ...
  add column if not exists rematch_match_id uuid null references public.multiplayer_matches(id) on delete set null;

create index if not exists idx_mp_players_rematch_match on public.multiplayer_match_players(rematch_match_id);
...
begin
  ...
  into v_total_count, v_requested_count
  from public.multiplayer_match_players
  where match_id = p_match_id
    and status = 'accepted'
