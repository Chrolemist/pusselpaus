...
begin
  ...
  if v_game_id in ('sudoku', 'numberpath') then
    ...
    limit 1;
  elsif v_game_id in ('rytmrush', 'pingpong') then
    select p.user_id
    into v_winner_id
    from public.multiplayer_match_players p
    where p.match_id = p_match_id
      ...
      select
        ...
        row_number() over (
          order by
            ...
            case
              when v_game_id in ('rytmrush', 'pingpong') then -coalesce(p.survived_seconds, 0)
              else extract(epoch from coalesce(p.submitted_at, now()))
            end asc,
            case when v_game_id in ('rytmrush', 'pingpong') then -coalesce(p.score, 0) else coalesce(p.elapsed_seconds, 2147483647) end asc,
            p.submitted_at asc,
            p.user_id asc
        ) as placement
      from public.multiplayer_match_players p
